...
**Phases 1 and 2 are done.** The strip list below has been executed, the layout
table is retail's, the data is regenerated against `valve/maps`, and the client
and world are repointed at the `hlap` mod folder. What is left is the game side:
Phase 0 (the spike) and Phases 3 to 6. `game/README.md` is the shape of it, and
the headers under `game/src/` are the interfaces.
...
Phase 0, the spike: an `hlap` folder with a dll built unmodified from the current
SDK that starts a retail single-player map through the fallback and prints to the
player. Everything else follows from it, and if it fails, try Metamod before
