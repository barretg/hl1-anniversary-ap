...
# Where the bridge lives: `<Half-Life>/hlap/archipelago`. Spelled out here rather
# than imported from the `mod` package so that this module keeps its one useful
# property -- it depends on nothing but the standard library, which is what lets
# `tests/test_bridge.py` exercise the whole protocol with no game and no server.
# `tests/test_mod_install.py` checks the two definitions have not drifted apart.
MOD_DIR = "hlap"
...
# of filler grants. Writing them all into every snapshot means the game
...
# game discards a DeathLink older than ten seconds, so one stuck behind a
...
class Bridge:
    ...
    def __init__(self, store_dir: str | os.PathLike[str]) -> None:
        ...
        # on every launch, so the game needs to know when that has happened or
    ...
    def read_events(self) -> list[GameEvent]:
        ...
        with self.out_path.open("r", encoding="utf-8", errors="replace") as handle:
            ...
            chunk = handle.read()
            # Only advance past data we actually consumed, so a line the game
    ...
    def write_snapshot(
        ...
        *,
        ...
        because `now=` had moved on made the game reparse and re-ACK the whole
    ...
    def _publish(self, text: str) -> None:
        ...
        if the destination is open in another process, and the game opens
        ...
        read is transient and the game re-reads on the next poll, whereas a
...
def find_store_dir(game_dir: str | os.PathLike[str]) -> Path:
    """Resolve `<Half-Life>/hlap/archipelago`.

    Accepts either the Half-Life install root or the `valve` directory itself,
    since both are reasonable things for someone to point at. The bridge lives in
    our own mod folder, never under `valve`.
    """
    root = Path(game_dir)
    if root.name.lower() == "valve" and not (root / "valve").is_dir():
        root = root.parent
    return root / MOD_DIR / STORE_SUBDIR


def is_game_dir(path: str | os.PathLike[str]) -> bool:
    """Does this look like a Half-Life installation?

    Checked against the campaign maps rather than just the folder name, so
    pointing at an empty `Half-Life` folder is rejected rather than silently
    ...
    try:
        ...
        return False
    if root.name.lower() == "valve":
        candidates = [root, root / "valve"]
    else:
        candidates = [root / "valve", root]
    return any((c / "maps" / "c0a0.bsp").is_file() for c in candidates)
